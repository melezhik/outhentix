...
cmp-ok $otx.results[0]<message>, '~~', /'text match' .* 'HELLO'/, 'correct message (plain check hello)';
cmp-ok $otx.results[0]<type>, 'eq', 'check-expression', 'correct type (plain check hello)';

cmp-ok $otx.results[1]<message>, '~~', /'text match' .* 'WORLD'/, 'correct message (plain check world)';
