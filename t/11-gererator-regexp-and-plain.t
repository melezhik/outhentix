...
cmp-ok $otx.results[0]<message>, '~~', /'text match' .* 'HELLO WORLD'/, 'correct message (regexp check)';
cmp-ok $otx.results[1]<message>, '~~', /'text match' .* '(\w+) \s+ (\d+)'/, 'correct message (regexp check)';
cmp-ok $otx.results[2]<message>, '~~', /'text match' .* 'three for 3'/, 'correct message (three for 3 check)';
